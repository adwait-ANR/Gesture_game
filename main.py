# main.py - full package: win32 strong injection, pause/resume, FPS, commented L/R, gestures
import os
...
# key control (provides win32 or pynput backends)
from key_control import start_repeat, stop_repeat, press_hold, release_hold, USE_WIN32

# key mapping: using VK codes for win32; when using pynput, main sets cfg keys to strings/Key objects
try:
...
for name in [n for n in dir() if n.isupper()]:
    try: setattr(cfg, name, globals()[name])
    except: pass

# Assign strong-mode key constants
if USE_WIN32:
    # Virtual-Key codes
    cfg.UP_KEY = 0x26   # VK_UP
    cfg.DOWN_KEY = 0x28 # VK_DOWN
    cfg.LEFT_KEY = 0x25 # VK_LEFT (commented usage)
    cfg.RIGHT_KEY = 0x27
else:
    # fallback to pynput values
    if PYNPUT_AVAILABLE:
        cfg.UP_KEY = Key.up
...
def show_flash_screen(frame, color, text, w, h, duration=700):
    ...
    cv2.putText(flash, text, (60,200), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (255,255,255), 5)
...
def do_gesture_calibration(detector, get_frame, logic, w, h):
    ...
    while True:
        ...
        if elapsed >= CAL_TIME: break
        f = get_frame()
        if f is None: continue
        f = cv2.flip(f,1)
        remaining = int(CAL_TIME - elapsed) + 1
        overlay = f.copy(); cv2.rectangle(overlay,(0,0),(w,h),(0,0,0),-1)
        f = cv2.addWeighted(overlay, 0.3, f, 0.7, 0)
        cv2.putText(f, 'GESTURE CALIBRATION', (50,100), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,0,255),3)
        cv2.putText(f, str(remaining), (w//2-30, h//2), cv2.FONT_HERSHEY_SIMPLEX, 4, (0,0,255),6)
        progress = int((elapsed / CAL_TIME) * (w - 100))
        cv2.rectangle(f, (50, h-60), (50+progress, h-20), (0,0,255), -1)
        cv2.rectangle(f, (50, h-60), (w-50, h-20), (255,255,255), 2)
        cv2.imshow('motion-tracker', f); cv2.waitKey(1)
        r = detector.process(cv2.cvtColor(f, cv2.COLOR_BGR2RGB))
        if r.pose_landmarks: samples.append(r.pose_landmarks.landmark)
    ...
    if hip_b is not None:
        ...
        print('[main] Gesture calibration OK'); show_flash_screen(frame, (0,255,0), 'CALIBRATION OK', w, h)
    else:
        print('[main] Gesture calibration FAILED'); show_flash_screen(frame, (0,0,255), 'CALIBRATION FAILED', w, h)

def main():
    already_closed = False
    stop_reader, get_frame = start_camera_reader(CAM_INDEX, WIDTH, HEIGHT, FLUSH_READS, CAP_PROP_BUFFERSIZE)
    detector = PoseDetector(model_complexity=MODEL_COMPLEXITY, min_detection_confidence=MIN_DETECTION_CONFIDENCE, min_tracking_confidence=MIN_TRACKING_CONFIDENCE)
        ...
        while True:
            ...
            if not calibrated:
                ...
                if remaining > 0:
                    ...
                    hip_b, cen_b = median_baseline_from_landmarks(calib_samples, w, h)
                    if hip_b is not None:
                        logic.set_baseline(hip_b, cen_b); calibrated = True
                        print('[main] Baseline OK:', hip_b, cen_b)
                    else:
                        print('[main] Auto calibration failed. Use L gesture to recalibrate.')
            ...
            if calibrated and result.pose_landmarks:
                ...
                # criss-cross exit
                if detect_criss_cross(lm, h, w):
                    print('Criss-cross detected -> Exiting safely.')
                    try:
                        stop_repeat(cfg.UP_KEY); stop_repeat(cfg.DOWN_KEY)
                    except: pass
                    try:
                        release_hold(cfg.UP_KEY); release_hold(cfg.DOWN_KEY)
                    except: pass
                    stop_reader()
                    try: detector.close()
                    except: pass
                ...
                if detect_pause_gesture(lm, h, w):
                    paused = not paused
                    if paused:
                        print('Pause gesture detected -> PAUSED')
                    else:
                        print('Pause gesture detected -> RESUMED')
                    # small debounce
                    time.sleep(0.6)

                if not paused:
                    frame = draw_stick_figure(frame, lm, h, w)
                    lh_y = lm[23].y * h; rh_y = lm[24].y * h
                    hip_y = (lh_y + rh_y) / 2.0
                    # center_x for potential left/right (commented use)
                    lsh = lm[11].x * w; rsh = lm[12].x * w
                    center_x = (lsh + rsh) / 2.0

                    action, status = logic.update(hip_y, center_x)

                    # Uncomment left/right logic when needed
                    ...
                    # else:
                    #     stop_repeat(cfg.LEFT_KEY); stop_repeat(cfg.RIGHT_KEY)

                    frame = draw_hip_thresholds(frame, logic, cfg)

                    # L-gesture recalibration
                    if detect_L_gesture(lm, h, w):
                        do_gesture_calibration(detector, get_frame, logic, w, h)
                        continue

            # HUD: status and FPS
            cv2.putText(frame, f'Status: {status}', (10, h-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
            cv2.putText(frame, f'FPS: {fps}', (w-120, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200,200,0), 2)
